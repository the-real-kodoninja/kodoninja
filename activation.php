...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])) {
	...
	$e = mysqli_real_escape_string($cnnc_t, $_GET['e']);
	$p = mysqli_real_escape_string($cnnc_t, $_GET['p']);
	...
	if($id == "" || strlen($u) < 3 || strlen($e) < 5 || $p == ""){
		...
		header("location: 404.php?msg=activation_string_length_issues");
    	exit(); 
	}
	...
	if($numrows == 0){
		...
		header("location: 404.php?msg=Your credentials are not matching anything in our system");
    	exit();
	}
    ...
    if($numrows == 0){
        ...
        header("location: 404.php?msg=activation_failure");
    	exit();
    } else if($numrows == 1) {
		// Great everything went fine with activation!
        header("location: 404.php?msg=activation_success");
    	exit();
    }
} else {
	// Log this issue of missing initial $_GET variables
	header("location: 404.php?msg=missing_GET_variables");
    exit(); 
}
?>